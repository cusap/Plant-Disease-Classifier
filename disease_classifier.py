...
if __name__ == '__main__':
    ...
    try:
        with tf.device('/device:GPU:0'):
            ...
            x = b_block(x, num_channels = 48, exp_fac=6, n=2, s=2)
            x = b_block(x, num_channels = 64, exp_fac=6, n=3, s=2)
            #x = b_block(x, num_channels = 128, exp_fac=6, n=4, s=2)
            ...
            im_gen = tf.keras.preprocessing.image.ImageDataGenerator(rotation_range=360,
                                                                     ...
                                                                     '''
            '''
            im_gen = tf.keras.preprocessing.image.ImageDataGenerator(rotation_range=0,
                                                                     zoom_range=[0.1, .1],
            ...
            opt = tf.keras.optimizers.RMSprop(learning_rate=learning_rate, decay=.9, momentum=.9)
            ...
            model_log = model.fit_generator(im_gen.flow(train_im, train_labels, batch_size=batch_size),
                                            ...
                                            validation_data=(val_im, val_labels), verbose=2)
            '''
            #model_log = model.fit(train_im, train_labels    )
