...
def process_picture(image):
    ...
    cv2.imwrite("segmented_leaf.jpg", image_rgb_nobg)
...
def disease_detector(img):
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            ...
            # print(text)
            cv2.imwrite("detected_disease.jpg", img)
...
def leaf_detector(img):
    ...
    # Show Original Image
    show_img = img
    cv2.imwrite("original-image.jpg", show_img)
    cv2.imshow("Image", cv2.resize(show_img, (256,256)))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            ...
            #             0.5, color, 2)
            # cv2.imwrite("leaf-detect.jpg", img)
    return(crop_boxes)
    ...
    if (len(bounding_boxes) > 0):
        ...
        for box in bounding_boxes:
            ...
            try:
                ...
                cv2.imwrite("cropped-leaf.jpg", crop_img)
                cv2.imshow("cropped", cv2.resize(crop_img, (256, 256)))
                cv2.waitKey(0)
                cv2.destroyAllWindows()
            except:
                continue
            crop_img = process_picture(crop_img)
            int_result = disease_detector(crop_img)
            if int_result != 0:
                try:
        ...
        # last = disease_detector(cv2.resize(img, (256, 256)))
        # if last != 0:
        #     results.append(last)
        if len(results) > 0:
...
if __name__ == "__main__":
    run_model("leaves/black_rot_90.jpg")
